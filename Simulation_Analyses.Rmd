...
Each of the **`r nrow(wholecounts)`** species has its own IgA value distribution. This is analgous to the differing IgA binding affinities of real taxa - which IgA-Seq aims to determine.
...
igabinding %>% ggplot(aes(x=IgAValue,color=Species))+geom_density()+geom_vline(xintercept = c(posthresh,negthresh),lty=2,col=c("darkgrey"))+scale_color_d3()+ylab("Density")+xlab("IgA Value")
...
wholel <- wholeabunds %>% rownames_to_column(var="Species") %>% gather("Sample","Abundance",-Species) %>% add_column(Fraction=rep("Pre-sort",nrow(.)))
...
combinedl <- rbind(wholel,posl,negl) %>% mutate(Sample=factor(Sample,levels=colnames(wholeabunds)), Species=factor(Species,levels=speclevs),Fraction=factor(Fraction,levels=c("Pre-sort","Pos","Neg")))

combinedl %>% ggplot(aes(x=Sample,y=Abundance,fill=Species))+geom_col()+facet_grid(.~Fraction)+scale_fill_d3()+coord_flip()+theme(axis.ticks.y=element_blank(), axis.text.y=element_blank())
...
fracsizes <- data.frame(Fraction=c(rep("Pos",length(possizes)),rep("Neg",length(negsizes))),Size=c(possizes,negsizes),Sample=rep(names(possizes),2)) %>% mutate(Sample=str_replace(Sample,"Sample","")) %>% mutate(Sample=factor(Sample,levels=c(1:ncol(wholeabunds))))
...
```{r make_scores}
#pseudo count is set by looking at the range of the minimum non-zero values in the pos, whole and neg abundance tables. The lowest being 1e-5
pseudo=1e-5
...
cvs <- rbind(getcov(palm,"Palm"),getcov(kau,"Kau"),getcov(posprob,"Positive Probability"),getcov(probrat,"Probability Ratio"))
cvs$Species <- factor(cvs$Species,levels=speclevs)
cvs %>% ggplot(aes(y=CV, x=Score))+geom_boxplot()+geom_point(aes(color=Species))+scale_y_log10()+scale_color_d3()+ylab("Coefficient of Variance")+guides(color=F)
...
corplotfun <- function(tab,meanvals,title){
  ...
  p <- ggplot(df,aes(y=True,x=Scores))+geom_point(aes(col=Species))+xlab("Species Mean IgA Value")+ggtitle(title)+stat_cor(method="spearman",label.y = 0,cor.coef.name="rho")+stat_cor(method="pearson",label.y =-0.25)+scale_color_d3()+guides(col=F)+theme_classic()+ylab("True IgA Value")+xlab("Score Value")
  return(p)
}
...
Relative abundances were generated from simulated counts. These will be used to generate scores. In this case, additional counts were added to **`r betweendat$expspecies`** in **Sample`r round(ncol(betweendat$wholeabunds)/2)`** to **Sample`r ncol(betweendat$wholeabunds)`**.

```{r gen_abund_bet, include=T, fig.width=8, fig.height=4.8}

wholelb <- betweendat$wholeabunds %>% rownames_to_column(var="Species") %>% gather("Sample","Abundance",-Species) %>% add_column(Fraction=rep("Pre-sort",nrow(.)))
...
combinedlb <- rbind(wholelb,poslb,neglb) %>% mutate(Sample=factor(Sample,levels=colnames(betweendat$wholeabunds)),Species=factor(Species,levels=rownames(betweendat$wholeabunds)),Fraction=factor(Fraction,levels=c("Pre-sort","Pos","Neg"))) %>% add_column(Group=ifelse(betweendat$expgroup[.$Sample]=="1","Control","Case"))

combinedlb %>% ggplot(aes(x=Sample,y=Abundance,fill=Species))+geom_col()+facet_grid(Group~Fraction,scales="free")+scale_fill_d3()+coord_flip()+theme(axis.ticks.y=element_blank(), axis.text.y=element_blank())
...
posprobb <- igascores(posabunds = betweendat$posabunds, possizes = betweendat$possizes, totabunds = betweendat$wholeabunds, method="prob")
...
betgroup <- function(score,groups,name){
  ...
  compp <- scorel %>% ggplot(aes(x=Species,y=Score))+geom_boxplot(aes(fill=Group))+scale_fill_manual(values=c("#BC0072","#13C300"))+ggtitle(name)+xlab("Species")
