...
kresall <- unlist(apply(kau,1,igafun,met=sampmeta)) %>% t() %>% data.frame() %>% rownames_to_column("taxon") 
kres <- kresall %>% filter(Any_Sig==1)
prresall <- unlist(apply(probrat,1,igafun,met=sampmeta)) %>% t() %>% data.frame() %>% rownames_to_column("taxon")
...
sigplot <- function(sigres,scores,met,name){
  ...
  longdifs <- sigres %>% select("taxon","W3","W5","W7") %>% gather("Time","Score",-taxon) %>% mutate(taxon=gsub("^([A-z_]*;){4}","",taxon))
  ...
  longscores <- scores %>% rownames_to_column("taxon") %>% gather("Sample","Score",-taxon) %>%
    ...
    mutate(taxon=gsub("^([A-z_]*;){4}","",taxon))
...
sigcomb <- comb %>% filter(PR_P<0.05|Kau_P<0.05) 
sigpplot <- sigcomb %>% ggplot(aes(y=PR_P,x=Kau_P))+geom_point()+geom_abline(slope=1,lty=2,col="red")+scale_y_continuous(limits=c(0,0.087))+scale_x_continuous(limits=c(0,0.087))+xlab("Kau Index p-value")+ylab("Probability Ratio p-value")
sigeffplot <- sigcomb %>% ggplot(aes(y=abs(PR_EffectSize),x=abs(Kau_EffectSize)))+geom_point()+geom_abline(slope=1,lty=2,col="red")+scale_y_continuous(limits=c(0,10.6))+scale_x_continuous(limits=c(0,10.6))+xlab("Absolute Kau Effect Size\n(SSMD MAL/CON)")+ylab("Absolute Probability Ratio\nEffect Size (SSMD MAL/CON)")
...
The Probability Ratio appears more powered to detect differences (lower p-values and higher mean differences). This plot will look at the coefficient of variation within each taxa and each experimental group (age*diet) to see if this is due to reduced variance in score estimates in the Probability Ratio, as seen in the simulations.
